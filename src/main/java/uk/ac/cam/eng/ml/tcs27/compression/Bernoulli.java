...
public class Bernoulli<X> extends SimpleMass<X>
  ...
  /** Demonstrates sampling from a Bernoulli distribution. */
  public static void main(String[] args) throws Exception {
    Random rnd = new Random();
    Bernoulli<String> coin = new Bernoulli<String>("heads","tails",0.6);
    System.out.println(coin);
    for (int k=0; k<10; k++) {
      System.out.print("  "+coin.sample(rnd));
    }
    System.out.println();
    System.out.println();
    System.out.println("Encoding test for tiny probability masses...");
    // minimum mass is around 1.0e-9, anything below will be rounded up
    Bernoulli<Character> rare = new Bernoulli<Character>('A','B',1.0e-100);
    System.out.println("Information content of A: "+rare.info('A')+" bits");
    System.out.println("Information content of B: "+rare.info('B')+" bits");
    CountingBitBuffer bb = new CountingBitBuffer();
    Arith ac = new Arith();
    ac.start_encode(bb);
    Encoder ec = new DebugEncoder(ac);
    rare.encode('B',ec); // high probability event
    rare.encode('A',ec); // low probability event
    rare.encode('A',ec); // low probability event
    rare.encode('B',ec); // high probability event
    rare.encode('B',ec); // high probability event
    ac.finish_encode();
    System.out.println("Encoded symbols B,A,A,B,B with "
                                        +bb.bitsWritten()+" bits.");
    /*
    for (int k=0; k<1000000; k++) {
      rare.encode('B',ac);
    }
    System.out.println("Encoded 100000 symbols "+'B'+" with "
                                        +bb.bitsWritten()+" bits.");
    */
    Arith adc = new Arith();
    adc.start_decode(BitReaderSequence.appendZeros(bb));
    Decoder dc = new DebugDecoder(adc);
    Character x = rare.decode(dc);
