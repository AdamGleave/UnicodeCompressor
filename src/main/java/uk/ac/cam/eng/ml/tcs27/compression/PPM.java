...
public class PPM<X> extends SimpleMass<X> implements DSP<X>, AdaptiveCode<X> {
  ...
  public static class Node<X> {
    ...
    /** Total symbol count.
      * Number of times this symbol has occurred in parent context. */
    int tcount = 0;
    ...
    public double p(X x, NodeSet<X> excl, Distribution<X> base) {
      if (children != null) {
        ...
        if (n > 0) {
          ...
          double esc = (b2*a1 + b1*a2*k) / (b2 * (a2*n + a1));
          if (vine != null) {
    ...
    public double logMass(X x, NodeSet<X> excl, Distribution<X> base) {
      if (children != null) {
        ...
        if (n > 0) {
          // escape symbol contribution...
          double esc = Math.log(b2*a1 + b1*a2*k) - Math.log(b2)
                                                 - Math.log(a2*n + a1);
          if (vine != null) {
    ...
    public void encode(X x, NodeSet<X> excl, Codable<X> base, Encoder ec) {
      ...
      if (l == -1) {
        ...
        if (n > 0) {
          ...
          ec.storeRegion(t - b2*a1 - b1*a2*k, t, t);  // code escape symbol
        }
        if (vine != null) {
