...
object SimpleUTF8Encoder {
  ...
  def tokenToBytes(token: SimpleToken) : Array[Byte] = token match {
    case SUnicodeCharacter(codePoint) => encode(codePoint)
    case SIllegalByte (byte) => Array(byte)
    case SEOF() => throw new AssertionError("EOF has no byte representation.")
  }
}